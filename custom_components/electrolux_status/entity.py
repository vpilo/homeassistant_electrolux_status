...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    if appliances is not None:
        for appliance_id, appliance in appliances.appliances.items():
            _LOGGER.debug("Electrolux add %d entities to registry for appliance %s", len(appliance.entities), appliance_id)
            async_add_entities(appliance.entities)

class ElectroluxEntity(CoordinatorEntity):
    appliance_status: ApplienceStatusResponse

    def __init__(self, coordinator: any, name, config_entry,
                 pnc_id, entity_type, entity_attr, entity_source, capability: dict[str, any]):
            ...
            if properties:
                ...
                if reported:
                    if self.entity_category:
                        category: dict[str, any]|None = reported.get(self.entity_category, None)
                        if category:
...
class ElectroluxButtonEntity(ElectroluxEntity):
    def __init__(self, coordinator, capability, name, config_entry, pnc_id, entity_type, entity_attr, entity_source, val_to_send, icon):
        super().__init__(coordinator, capability, name, config_entry, pnc_id, entity_type, entity_attr, entity_source)
